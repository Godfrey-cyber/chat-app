      ...
      {messages.map((message, i) => {
        ...
        return (
            <div key={i} className="flex flex-col">
              {isSentByCurrentUser ? (
              <div key={i} className="flex flex-col rounded-2xl w-4/5 bg-teal-400 justify-self-end ml-4 my-3 px-4">
                  <p className="text-orange-600 pl-3 text-xs pt-2">{trimmedName}</p>
                  <p className="text-gray-600 rounded-full text-xs p-2">{ text }</p>
                </div>
              ) : (
                <div key={i} className="flex flex-col rounded-2xl w-4/5 bg-slate-800 justify-self-start ml-4 my-3 px-4">
